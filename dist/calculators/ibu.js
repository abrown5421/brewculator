...
function getUtilization(gravity, boilTime) {
    const nearestGravity = gravityKeys.reduce((prev, curr) => Math.abs(curr - gravity) < Math.abs(prev - gravity) ? curr : prev);
    const nearestTime = timeKeys.reduce((prev, curr) => Math.abs(curr - boilTime) < Math.abs(prev - boilTime) ? curr : prev);
...
async function ibu() {
    ...
    try {
        const preBoilSize = Number(await (0, prompts_1.input)({
            message: "Enter Pre Boil Size (gal):",
            validate: (value) => isNaN(Number(value)) ? "Must be a number" : true,
        }));
        const postBoilSize = Number(await (0, prompts_1.input)({
            message: "Enter Post Boil Size (gal):",
            validate: (value) => isNaN(Number(value)) ? "Must be a number" : true,
        }));
        const targetOg = Number(await (0, prompts_1.input)({
        ...
        while (addMoreHops) {
            ...
            const ounces = Number(await (0, prompts_1.input)({
                message: "Enter ounces:",
                validate: (value) => isNaN(Number(value)) ? "Must be a number" : true,
            }));
            const alphaAcid = Number(await (0, prompts_1.input)({
                message: "Enter Alpha Acid (%):",
                validate: (value) => isNaN(Number(value)) ? "Must be a number" : true,
            }));
            const boilTime = Number(await (0, prompts_1.input)({
                message: "Enter Boil Time (minutes):",
                validate: (value) => isNaN(Number(value)) ? "Must be a number" : true,
            }));
            hops.push({ ounces, alphaAcid, boilTime });
            addMoreHops = await (0, prompts_1.confirm)({
