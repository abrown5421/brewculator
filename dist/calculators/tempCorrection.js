...
exports.tempCorrection = tempCorrection;
const prompts_1 = require("@inquirer/prompts");
async function tempCorrection() {
    console.clear();
    console.log("Hydrometer Temperature Correction\n");
    try {
        const measuredGravity = Number(await (0, prompts_1.input)({
            message: "Enter measured gravity (e.g., 1.050):",
            validate: (value) => isNaN(Number(value)) ? "Must be a number" : true,
        }));
        const measuredTemp = Number(await (0, prompts_1.input)({
            message: "Enter wort temperature (°F):",
            validate: (value) => isNaN(Number(value)) ? "Must be a number" : true,
        }));
        const calibrationTemp = Number(await (0, prompts_1.input)({
        ...
        if (Math.abs(pointsDifference) < 0.5) {
            ...
            console.log("⚠️  Hot wort reads lower - actual gravity is higher");
        }
        else {
            console.log("⚠️  Cold wort reads higher - actual gravity is lower");
        }
